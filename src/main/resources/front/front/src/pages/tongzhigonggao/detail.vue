	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/tongzhigonggao?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">公告类型</div>
						<div class="text "  >{{detail.gonggaoleixing}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">公告时间</div>
						<div class="text "  >{{detail.gonggaoshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('tongzhigonggao','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('tongzhigonggao','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.gonggaotupian) {
							this.detailBanner = this.detail.gonggaotupian.split(",w").length>1?[this.detail.gonggaotupian]:this.detail.gonggaotupian.split(',');
						}
						this.$forceUpdate();
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/tongzhigonggao', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此通知公告？') .then(_ => {
					this.$http.post('tongzhigonggao/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
