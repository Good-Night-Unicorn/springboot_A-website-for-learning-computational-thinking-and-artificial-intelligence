<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="教师职称" prop="jiaoshizhicheng">
				<el-input v-model="ruleForm.jiaoshizhicheng" 
					placeholder="教师职称" clearable :disabled=" false  ||ro.jiaoshizhicheng"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='jiaoshizhicheng'){
							this.ruleForm.jiaoshizhicheng = obj[o];
							this.ro.jiaoshizhicheng = true;
							continue;
						}
						if(o=='touxiang'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`laoshi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`laoshi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
