<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="完成附件" prop="wanchengfujian">
				<file-upload
					tip="点击上传完成附件"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" label="完成内容" prop="wanchengneirong">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="完成内容"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`tijiaozuoye/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`tijiaozuoye/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
