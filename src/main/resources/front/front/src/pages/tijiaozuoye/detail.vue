	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/tijiaozuoye?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">完成内容</div>
						<div class="text "  >{{detail.wanchengneirong}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">完成附件</div>
						<el-button class="uploadBtn" @click="download(detail.wanchengfujian )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('tijiaozuoye','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('tijiaozuoye','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('tijiaozuoye','批改')" 
							@click="onAcross('chengjixinxi','','','','')" type="warning">
							批改
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`tijiaozuoye`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/tijiaozuoye', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/tijiaozuoyeAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此提交作业？') .then(_ => {
					this.$http.post('tijiaozuoye/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
