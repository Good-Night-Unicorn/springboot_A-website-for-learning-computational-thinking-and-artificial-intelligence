	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xuankexinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">选课时间</div>
						<div class="text "  >{{detail.xuankeshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">选课备注</div>
						<div class="text "  >{{detail.xuankebeizhu}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xuankexinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xuankexinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('xuankexinxi','课程作业')" 
							@click="onAcross('buzhizuoye','','','','')" type="warning">
							课程作业
						</el-button>
					</div>
				</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/xuankexinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xuankexinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此选课信息？') .then(_ => {
					this.$http.post('xuankexinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
