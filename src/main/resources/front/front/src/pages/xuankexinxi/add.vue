<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="选课时间" prop="xuankeshijian">
				<el-date-picker
					:disabled=" false  ||ro.xuankeshijian"
			...
			<el-form-item class="add-item" label="选课备注" prop="xuankebeizhu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="选课备注"
					v-model="ruleForm.xuankebeizhu">
					</el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`xuankexinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xuankexinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
