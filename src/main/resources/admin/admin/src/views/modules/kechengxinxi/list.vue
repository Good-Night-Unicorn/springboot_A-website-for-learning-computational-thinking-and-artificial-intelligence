		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"alignItems":"center","padding":"10px","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","width":"120px","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">课程附件</label>
						<el-input v-model="searchForm.kechengfujian" placeholder="课程附件" @keydown.enter.native="search()" clearable></el-input>
					</div>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' prop="kechengfujian" label="课程附件">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','选课')" @click="xuankexinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								选课
							</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			xuankexinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
			...
			getDataList() {
				...
				if(this.searchForm.kechengfenlei!='' && this.searchForm.kechengfenlei!=undefined){
 				...
 				if(this.searchForm.kechengfujian!='' && this.searchForm.kechengfujian!=undefined){
					params['kechengfujian'] = '%' + this.searchForm.kechengfujian + '%'
				}
